"""
LOS pipeline transformer for Lilly's LOS XGBoost pipeline.

Must live at models/length_of_stay/transforms.py relative to the Python path root
so that pickle can find models.length_of_stay.transforms.LosTrainPreprocessor
when deserializing los_pipeline.pkl.

Standalone: constants from los_model_config are inlined here so the Baptist_tester
import chain is not needed at API runtime.
"""
...
# Inlined from los_model_config.TRANSFORMS where t == "log1p"
LOG1P_COLUMNS: list[str] = ["clinical_event_n_12h"]
# Inlined from los_model_config.TRANSFORMS where t == "sqrt"
...
# Inlined from los_model_config.MODEL_CATEGORICAL_FEATURES
CATEGORICAL_COLUMNS: list[str] = ["admit_type_cd", "admit_src_cd", "unit_cd"]
# Inlined from los_model_config.WINSORIZE_TRAIN_PCT
...
# Inlined from features.RESIDUAL_AUX_COLUMNS
RESIDUAL_AUX_COLUMNS: frozenset[str] = frozenset({"med_infusion_mean_12h", "current_scai_12h"})
